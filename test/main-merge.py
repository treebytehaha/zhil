...
from langchain.callbacks.manager import CallbackManagerForLLMRun  # 新增
...
def plot_chart(
    ...
    if chart_type == "line":
        chart = (
            Line()
            .add_xaxis(labels)
            .add_yaxis("", values, is_smooth=False, symbol="circle")
            # .set_global_opts(title_opts=opts.TitleOpts(title=title or ""))
        )
    elif chart_type == "pie":
        chart = (
            Pie()
            .add("", [list(z) for z in zip(labels, values)], radius="65%")
            # .set_global_opts(title_opts=opts.TitleOpts(title=title or ""))
        )
    else:  # bar
        chart = (
    ...
    # 2) 渲染为 PNG（snapshot-selenium）
    tmp_dir = tempfile.mkdtemp()
    png_path = os.path.join(tmp_dir, "tmp.png")
    make_snapshot(snapshot, chart.render(), png_path)

    with open(png_path, "rb") as f:
        img_bytes = f.read()

    # 3) 保存到会话目录，生成 image_url（和你原来的逻辑一致）
    image_url = None
    if session_id and session_id in sessions:
        session_dir = os.path.join(UPLOAD_DIR, session_id)
        os.makedirs(session_dir, exist_ok=True)
        doc_id = str(uuid.uuid4())
        filename = f"{doc_id}_chart.png"          # 改为 png
        save_path = os.path.join(session_dir, filename)
        with open(save_path, "wb") as f:
            f.write(img_bytes)
        sessions[session_id]["docs"][doc_id] = {
...
SYSTEM_PROMPT = """
你是一个专业的 Excel 数据分析和知识问答助手。你可以回答多轮对话的问答，并且记住之前对话的所有内容，回答前要先检索记忆模块，然后再回答。
...
Always use *已知事实* 中的内容回答。如果用户问
“我的名字是什么”等问题，而已知事实中包含名字，
直接引用，不要回答 “您没告诉我”。
"""
# ========================
# 自定义 DashScope LLM
...
tools = [
    ...
    StructuredTool.from_function(
        ...
        name="column_top_k",
        description="统计某列中出现次数最多的前 k 个取值,请你注意，一定要在没有时间信息的情况下使用该工具，例如\"Offer列中的前6个变量\"，则可以调用该工具",
    ...
    StructuredTool.from_function(
        ...
        description="统计某列中特定取值的出现次数，请你注意，一定要在没有时间信息的情况下使用该工具，例如你需要获取某列中某个变量出现了多少次，则可以调用该工具",
...
def build_agent():
    ...
    entity_mem = ConversationEntityMemory(llm=llm, memory_key="entities")
    long_mem   = VectorStoreRetrieverMemory(retriever=vectorstore.as_retriever(search_kwargs={"k":3}))

    return initialize_agent(
        llm=llm,
        tools=tools,
        agent=AgentType.STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION,  # 支持多参数工具
        max_iterations=3,
        early_stopping_method="generate",
        return_source_documents=False,
        verbose=True,
        memory=short_mem,
        additional_memories=[entity_mem, long_mem],
...
async def create_session():
    ...
    # 用 initialize_agent（结构化）来同时支持多参数工具 + memory
...
async def analyze_in_session(session_id: str, req: AnalyzeInSession):
    ...
    short_mem = sessions[session_id]["short_mem"]
...
# 2) 手动拼到本轮 query
        manual_prompt = f"{history}\nUser: 文件路径是 {file_path}，{req.command}"

# 3) 直接把整个拼好的 prompt 当作“用户输入”给 agent
#    并且这时要把 memory 关掉或清空，避免重复
        
        
        print("memory:运行前", agent.memory.chat_memory.messages)  # 调试输出短期记忆
        #print("long memory:", long_mem.retriever.get_relevant_documents(query))  # 调试输出长期记忆
        result = agent.run(manual_prompt, callbacks=[recorder])
        print("memory:运行后", agent.memory.chat_memory.messages)  # 调试输出短期记忆
        #print("long memory:", long_mem.retriever.get_relevant_documents(query))  # 调试输出长期记忆

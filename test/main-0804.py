...
prompt = ChatPromptTemplate.from_messages([
    ...
    SystemMessagePromptTemplate.from_template("—— 对话历史 ——\n{chat_history}"),
...
class DashScopeLLM(LLM):
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        ...
        print("LLM RESPONSE:", json.dumps(resp, ensure_ascii=False, indent=2))
...
chain = LLMChain(
    llm=llm,
    prompt=prompt,
    memory=short_term_memory,          # ← 短期记忆 Buffer
)
...
import langchain.agents.structured_chat.agent
from langchain.agents.structured_chat.agent import StructuredChatAgent
from langchain.agents.structured_chat.output_parser import StructuredChatOutputParserWithRetries
from langchain.agents import AgentExecutor
agent_def = StructuredChatAgent.from_llm_and_tools(
    llm_chain=chain,
    tools=tools,
